#!/usr/bin/env python3
"""Extract squeeze_module (+ bb_norm3 for context) weights from full BiRefNet GGUF."""
from __future__ import annotations

import argparse
import os
import sys

import numpy as np

try:
    import gguf
except ImportError:
    sys.exit("pip install gguf")

PREFIXES = (
    "squeeze_module_0_",
    "bb_norm3_",
)


def shorten(name: str) -> str:
    name = name.replace("squeeze_module_0_", "sq0_", 1)
    if len(name) >= 64:
        sys.exit(f"name still too long after shorten: {name} ({len(name)})")
    return name


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--in", dest="inp", required=True)
    ap.add_argument("--out", required=True)
    args = ap.parse_args()

    reader = gguf.GGUFReader(args.inp)
    writer = gguf.GGUFWriter(args.out, "rmbg")
    writer.add_string("general.architecture", "rmbg")
    writer.add_string("rmbg.subset", "squeeze_module")
    n = 0
    for t in reader.tensors:
        if not t.name.startswith(PREFIXES):
            continue
        out_name = shorten(t.name) if t.name.startswith("squeeze_module_0_") else t.name
        arr = np.array(t.data, dtype=np.float32)
        writer.add_tensor(out_name, arr)
        n += 1
    if n == 0:
        sys.exit("no tensors matched")
    writer.write_header_to_file()
    writer.write_kv_data_to_file()
    writer.write_tensors_to_file()
    writer.close()
    print(f"wrote {args.out} ({n} tensors, {os.path.getsize(args.out)/1024/1024:.1f} MB)")


if __name__ == "__main__":
    main()
