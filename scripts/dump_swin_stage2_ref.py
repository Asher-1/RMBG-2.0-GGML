#!/usr/bin/env python3
"""Dump Swin stage-2 output (through 18 blocks @ 64x64)."""
...
def main():
    ...
    ap.add_argument("--out", default=os.path.join(ROOT, "tests", "fixtures", "swin_stage2_ref.gguf"))
    ...
    with torch.no_grad():
        ...
        H2, W2 = H1 // 2, W1 // 2
        s2 = run_stage(bb.layers[2], tokens, H2, W2, ws)
    ...
    writer.close()
    print(f"wrote {args.out} shape {s2.shape}")
